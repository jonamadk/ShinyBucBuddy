...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('chromadb_push.log')  # Local log file
    ]
)
...
chroma_client = chromadb.HttpClient(
    host="localhost",  # Use localhost since running locally
    port=8000,        # Use the host port from docker-compose.yml
    settings=Settings(allow_reset=True)
)

# Define embedding function (optional, but consistent with your usage)
...
def get_embedding(text, model="text-embedding-3-large"):
    ...
    try:
        ...
        logger.error("Failed to generate embedding for text '%s': %s", text[:50], str(e))
        raise
...
try:
    ...
    logger.info("Collection 'web_information' accessed or created successfully")
except Exception as e:
    logger.error("Failed to access or create collection 'web_information': %s", str(e))
    raise
...
for item in data:
    ...
    try:
        tag1, tag2, tag3, tag4, tag5, description = item.get('metadata', ['', '', '', '', '', ''])
        metadata_text = f"{tag1}, {tag2}, {tag3}, {tag4}, {tag5}, {description}"
    except (IndexError, TypeError):
        metadata_text = ""

    document_title = item.get('document_title', 'No title')
    document_link = item.get('document_link', 'Source Link has expired. Cross-validate the response')
    ...
    try:
        # Generate embedding
        embedding = get_embedding(combined_text, model="text-embedding-3-large")
        ...
        total_items += 1
        logger.info("Item %d pushed to ChromaDB with ID %s", item_count, doc_id)
    except Exception as e:
